...
class DataTable:
    ...
    def __init__(
        ...
        on_row_click: Optional[Callable] = None,  # Accept the new handler
        show_actions: bool = True,
    ):
        self.state = state
        self.on_edit = on_edit
        self.on_delete = on_delete
        self.on_row_click = on_row_click  # Store the handler
        ...
        with self.container:
            ...
            with ui.card().classes("w-full"):
                ...
                with ui.row().classes("w-full bg-gray-100 p-2"):
                    for column in columns:
                        with ui.row().classes("flex-1 items-center cursor-pointer").on(
                            "click", lambda c=column: self._sort_by_column(c)
                        ):
                ...
                for record in records:
                    # Apply a pointer cursor if rows are clickable
                    ...
                    # Make the entire row clickable if a handler is provided
                    if self.on_row_click:
        ...
        with ui.row().classes("w-full justify-center items-center gap-2 p-2"):
            ...
            with ui.row().classes("items-center gap-2"):
                ...
                ui.number(
                    ...
                    max=total_pages,
                    on_change=lambda e: (
                        self._go_to_page(int(e.value)) if e.value else None
                    ),
