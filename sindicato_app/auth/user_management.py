...
class UserManagementView:
    ...
    def create(self) -> ui.column:
        ...
        with container:
            ...
            if not self.has_role('admin', 'sistemas'):
                ui.label('Acceso denegado. Se requieren permisos de administrador.').classes(
        ...
        with ui.card().classes('w-full'):
            ...
            for user in self.users_list:
                with ui.row().classes('w-full border-b p-3 hover:bg-gray-50 items-center'):
                    ...
                    roles_label = ui.label('Cargando...').classes('flex-2 text-gray-500')
                    ui.timer(0.1, lambda u=user, rl=roles_label: self._load_user_roles_display(u, rl), once=True)

                    with ui.row().classes('w-40 gap-1'):
                        ui.button(
                            icon='edit',
                            on_click=lambda u=user: self._open_edit_user_dialog(u)
                        ).props('size=sm flat dense color=orange-600').tooltip('Editar usuario')

                        ui.button(
                            icon='security',
                            on_click=lambda u=user: self._open_roles_dialog(u)
                        ).props('size=sm flat dense color=blue-600').tooltip('Gestionar roles')

                        ui.button(
                            icon='key',
                            on_click=lambda u=user: self._open_password_dialog(u)
                        ).props('size=sm flat dense color=green-600').tooltip('Cambiar contraseña')
        ...
        with dialog, ui.card().classes('w-96'):
            ...
            with ui.row().classes('w-full justify-end gap-2 mt-4'):
                ...
                async def create_user():
                    # Validation
                    if not alias_input.value or not nombre_input.value or not password_input.value:
                    ...
                    try:
                        ...
                        user_data = {
                            ...
                            'apellidos': apellidos_input.value.strip() if apellidos_input.value else '',
        ...
        with dialog, ui.card().classes('w-96'):
            ...
            with ui.row().classes('w-full justify-end gap-2 mt-4'):
                ...
                async def update_user():
                    ...
                    try:
                        result = await self.api.update_record('usuarios', user['id'], user_data)
                        if result:
    ...
    async def _open_roles_dialog(self, user: Dict):
        ...
        with dialog, ui.card().classes('w-96'):
            ui.label(f'Gestionar Roles: {user.get("alias", "")}').classes('text-h6 mb-4')

            # Current roles display
            current_roles = [role['nombre'] for role in self.roles_list if role['id'] in current_role_ids]
            ...
            with ui.row().classes('w-full justify-end gap-2 mt-4'):
                ...
                async def update_roles():
                    try:
                        # FIXED: Delete existing user-role links properly
                        # Since usuario_roles has a composite primary key (usuario_id, role_id),
                        # we need to delete each link individually
                        existing_links = await self.api.get_records(
                            "usuario_roles",
                            {"usuario_id": f"eq.{user['id']}"}
                        )

                        # Delete each existing role assignment
                        for link in existing_links:
                            # For composite keys, we need a custom delete
                            # Using the raw client to handle composite key deletion
                        ...
                        ui.notify(f'Error al actualizar roles: {str(e)}', type='negative')

                ui.button('Actualizar Roles', on_click=update_roles).props('color=orange-600')
            ...
            with ui.row().classes('w-full justify-end gap-2 mt-4'):
                ...
                async def change_password():
                    ...
                    try:
                        ...
                        # FIXED: Update password hash using correct primary key
                        ...
                        if cred_records:
                            ...
                            # Since usuario_credenciales uses usuario_id as primary key
                            client = self.api._ensure_client()
                            ...
                            result = await self.api.create_record('usuario_credenciales', cred_data)
                            if result:
                                ui.notify('Contraseña creada exitosamente', type='positive')
                                dialog.close()
                            else:
                                ui.notify('Error al crear la contraseña', type='negative')

                    except Exception as e:
                        ui.notify(f'Error al cambiar contraseña: {str(e)}', type='negative')
                        print(f"Password change error for user {user['id']}: {str(e)}")  # Debug log
                ...
                with roles_container:
                    for role in self.roles_list:
                        with ui.row().classes('w-full items-center justify-between'):
                            ...
                            ui.button(
                                icon='delete',
                                on_click=lambda r=role: self._delete_role(r, refresh_roles)
