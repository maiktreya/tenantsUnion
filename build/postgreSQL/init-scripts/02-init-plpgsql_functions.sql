...
-- pgcrypto: required for server-side bcrypt verification in rpc_login
-- (audit item #3a — see migrations/05_rpc_login_security_definer.sql).
...
-- =====================================================================
-- FUNCTION + TRIGGER: set_piso_provincia_from_cp  (trigger_b)
-- =====================================================================
-- Deriva pisos.provincia_id a partir de los dos primeros dígitos del CP
-- (código INE de provincia), buscando el provincias.id cuyo code coincide.
--
-- ORDEN DE DISPARO: se llama trigger_b_set_provincia_from_cp para que,
-- por orden alfabético, se ejecute DESPUÉS de trigger_a_extract_cp
-- (que extrae NEW.cp desde direccion). Así, cuando un cliente inserta un
-- piso sin cp pero con direccion que contiene un CP de 5 dígitos, el
-- trigger_a fija NEW.cp primero y este trigger_b resuelve la provincia.
--
-- ALCANCE: BEFORE INSERT OR UPDATE ON pisos (no 'UPDATE OF cp') porque el
-- trigger_a puede mutar NEW.cp a consecuencia de un UPDATE de direccion;
-- si limitáramos a 'UPDATE OF cp' perderíamos la sincronización en ese
-- escenario. cp es la fuente de verdad: el valor derivado siempre
-- prevalece. Un cp no resoluble (NULL, prefijo '00', o code ausente) deja
-- provincia_id intacto para no destruir datos por un CP sucio.
-- =====================================================================

DROP FUNCTION IF EXISTS set_piso_provincia_from_cp() CASCADE;

CREATE OR REPLACE FUNCTION set_piso_provincia_from_cp()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    IF NEW.cp IS NOT NULL THEN
        -- Normaliza cp INTEGER a 5 dígitos (LPAD), extrae los 2 primeros,
        -- descarta '00' y convierte a int quitando ceros a la izquierda.
        v_code := NULLIF(
                      SUBSTRING(LPAD(NEW.cp::text, 5, '0') FROM 1 FOR 2),
                      '00'
                  )::int;
        IF v_code IS NOT NULL THEN
            SELECT p.id
              INTO v_prov_id
              FROM sindicato_inq.provincias p
             WHERE p.code = v_code;
            IF v_prov_id IS NOT NULL THEN
                NEW.provincia_id := v_prov_id;
            END IF;
        END IF;
    END IF;
    RETURN NEW;
...
DROP TRIGGER IF EXISTS trigger_b_set_provincia_from_cp ON sindicato_inq.pisos;

CREATE TRIGGER trigger_b_set_provincia_from_cp
    BEFORE INSERT OR UPDATE ON sindicato_inq.pisos
    FOR EACH ROW
    EXECUTE FUNCTION set_piso_provincia_from_cp();

-- =====================================================================
-- FUNCTION: rpc_login  (SECURITY DEFINER login endpoint)
-- =====================================================================
-- Replaces the previous client-side login flow (select usuarios +
-- usuario_credenciales + usuario_roles + roles as web_anon, which leaked
-- every bcrypt hash and every user alias to the public API). Now the
-- unauthenticated login view calls /rpc/rpc_login once; the function
-- verifies bcrypt server-side via pgcrypto's crypt() and returns
-- (user_id, alias, roles[]) on success, or empty (no row) on any failure.
--
-- SECURITY DEFINER + SET search_path so it bypasses RLS and is robust to
-- search_path injection. It is the only entry point that reads the
-- now-RLS-hardened usuarios / usuario_credenciales tables as an anonymous
-- caller; anonymous table-level SELECT on those is REVOKE'd (see
-- 06-init-rls.sql section 2/3 — patch in migrations/05).
--
-- Timing-safe: on the miss path (user not found / inactive / no
-- credentials row) we still run one crypt() round against a dummy bcrypt
-- hash so the response time is comparable to a real verify.
--
-- Full migration notes: migrations/05_rpc_login_security_definer.sql.
-- RLS / GRANT EXECUTE wiring is applied in 06-init-rls.sql (which is run
-- AFTER this script, per docker-entrypoint alphabetical ordering).
CREATE OR REPLACE FUNCTION sindicato_inq.rpc_login(
    p_alias    TEXT,
    p_password TEXT
)
RETURNS TABLE(
...
DECLARE
    ...
    v_roles TEXT[];
BEGIN
    -- Look up the user (case-insensitive, trimmed), mirroring the
    -- login form's normalisation.
    SELECT u.* INTO v_row
      FROM sindicato_inq.usuarios u
     WHERE LOWER(BTRIM(u.alias)) = LOWER(BTRIM(p_alias))
     LIMIT 1;

    -- Always perform one bcrypt round, even on the miss path, to mask
    -- the user-not-found vs bad-password distinction (timing side-channel).
    -- NB: the dummy salt MUST be `$2a$` not `$2b$` — pgcrypto's bundled
    -- crypt-blowfish doesn't recognize `$2b$` and would silently fall
    -- back to a much-faster legacy DES round, defeating the mitigation.
    IF v_row.id IS NULL OR v_row.is_active IS FALSE THEN
        PERFORM crypt(p_password,
            '$2a$12$CgNDkQp2XzR7YHw5r2mUu.uTbqX7vJlqJr9kK0Z6u3jxN0sJ8N1q');
        RETURN;
    END IF;

    SELECT c.password_hash INTO v_hash
      FROM sindicato_inq.usuario_credenciales c
     WHERE c.usuario_id = v_row.id;
    ...
    -- pgcrypto 1.3's bundled crypt-blowfish DOES NOT accept the `$2b$`
    -- version ident that passlib emits by default (it treats the salt
    -- as a legacy DES salt and returns 13 chars of garbage). It does
    -- accept `$2a$` correctly. The two idents are byte-equivalent for
    -- any real-world password length (the OpenBSD `$2b$ re-versioning`
    -- only affected inputs >255 bytes), so we normalize `$2b$` -> `$2a$`
    -- before invoking crypt() and compare against the normalized form.
    -- This makes stored passlib hashes (`$2b$12$...`) verify correctly.
    -- No storage-side migration needed.
    IF LEFT(v_hash, 4) = '$2b$' THEN
    ...
    -- Success: fetch role names so login is a single round trip. The
    -- client no longer needs the separate _get_user_roles helper that
    -- used to query usuario_roles + roles as web_anon.
    SELECT COALESCE(ARRAY_AGG(r.nombre) FILTER (WHERE r.nombre IS NOT NULL), ARRAY[]::TEXT[])
      INTO v_roles
      FROM sindicato_inq.usuario_roles ur
      JOIN sindicato_inq.roles r ON r.id = ur.role_id
     WHERE ur.usuario_id = v_row.id;
