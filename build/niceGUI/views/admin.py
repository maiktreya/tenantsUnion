...
class AdminView(BaseView):
    ...
    def create(self) -> ui.column:
        ...
        with container:
            ...
            with ui.row().classes("w-full gap-4 items-center"):
                ...
                # --- THIS BUTTON NOW CALLS THE SHARED METHOD ---
                ui.button("Limpiar Selección", icon="clear_all", on_click=self.clear_view_internals).props("color=grey-6")
    ...
    # The local _clear_view method is no longer needed; logic is now in BaseView.
    ...
    async def _load_table_data(self, table_name: str = None):
        ...
        with self.data_table_container:
            ...
            try:
                ...
                self._setup_filters()
                DataTable(
                    state=self.state, on_edit=self._edit_record,
                    on_delete=self._delete_record, on_row_click=self._on_row_click
                ).create()
    ...
    async def _on_row_click(self, record: Dict):
        ...
        await self.relationship_explorer.show_details(record, self.state.selected_entity_name.value, "admin")

    def _setup_filters(self):
        """Initializes the filter panel based on the loaded data."""
        self.state.filter_container.clear()
        with self.state.filter_container:
            self.filter_panel = FilterPanel(records=self.state.records, on_filter_change=self._update_filter)
    ...
    def _open_import_dialog(self):
        ...
        dialog = CSVImporterDialog(api=self.api, table_name=self.state.selected_entity_name.value, on_success=self._refresh_data)
        dialog.open()
    ...
    async def _create_record(self):
        ...
        if not self.state.selected_entity_name.value:
            return ui.notify("Por favor, seleccione una tabla primero", type="warning")
        dialog = EnhancedRecordDialog(api=self.api, table=self.state.selected_entity_name.value, mode="create", on_success=lambda: ui.timer(0.1, self._refresh_data, once=True))
        await dialog.open()

    async def _edit_record(self, record: Dict):
        """Opens a dialog to edit an existing record."""
        dialog = EnhancedRecordDialog(api=self.api, table=self.state.selected_entity_name.value, record=record, mode="edit", on_success=lambda: ui.timer(0.1, self._refresh_data, once=True))
        await dialog.open()
    ...
    async def _confirm_delete(self, record_id: int):
        ...
        if await self.api.delete_record(self.state.selected_entity_name.value, record_id):
    ...
    def _export_data(self):
        ...
        if self.state.selected_entity_name.value:
            export_to_csv(self.state.filtered_records, f"{self.state.selected_entity_name.value}_export.csv")
