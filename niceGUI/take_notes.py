...
def main_page():
    ...
    def update_headquarters_options():
        ...
        hq_select.set_options(COMPANIES.get(selected_company, []), clearable=True)
        hq_select.update()
    ...
    with ui.card().classes("w-full max-w-4xl mx-auto mt-8"):
        ui.label("Step 1: Select Company and Headquarters").classes(
        ...
        with ui.row().classes("w-full items-center"):
            ...
            hq_select = ui.select([], label="Headquarters").classes("w-64")

    # Note-taking section - This card will only appear when a headquarters is selected.
    with ui.card().classes("w-full max-w-4xl mx-auto mt-4").bind_visibility_from(
        hq_select, "value", bool
    ):
        ui.label("Step 2: Write and Save Your Note").classes("text-xl font-semibold")

        # Large text area for notes
        note_content = (
            ui.textarea(placeholder="Write your meeting minutes or notes here...")
    ...
    with ui.card().classes("w-full max-w-4xl mx-auto mt-4"):
        ...
        records_table = ui.table(
            columns=columns, rows=records, row_key="timestamp"
        ).classes("w-full")
