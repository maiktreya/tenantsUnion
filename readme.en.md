...
graph TD
    ...
    D --> E
    E <--> F
    D -->|On-demand Geolink Enrichment| I
...
- **Ingestion Automation (Headless ETL):** A modular background routine scheduled via `utils/cron/daily_sync.sh` that securely tunnels into remote target environments, isolates new registration records, standardizes unstructured user-typed strings, attaches geographic coordinate markers, and commits updates to PostgreSQL. That same geocoding logic is also available on demand from the interface, in the Geolink Enrichment module described below.

---

You can check a full ERD of the database at the following [link](https://github.com/maiktreya/tenantsUnion/blob/main/doc/ERD_database.png). It's just 16 base tables!
...
- **Database Administration Console (`ADMIN BBDD`):**
  - Full multi-table Create, Read, Update, and Delete (CRUD) controls with integrated foreign key lookups and contextual selection menus.
...
- **Data Import & Enrichment (`IMPORTAR DATOS`):**
  - **Generic CSV importer:** self-documenting column templates, automatic relational mapping across tables, and a real-time progress console during insertion.
  - **Automatic Piso → Bloque linker:** suggests candidate buildings by address similarity (`pg_trgm`) and lets you bulk-link every apartment above an adjustable confidence threshold in a single step.
  - **On-demand Geolink enrichment:** filters apartments missing a cadastral reference or coordinates and re-queries them against the CartoCiudad API, reusing the same geocoding logic as the nightly ETL pipeline.
...
- **Automated Spatial ETL Engine (Gravity Forms Synchronization):**
  - An unattended daily synchronization job (`utils/cron/daily_sync.sh`, 02:00) that extracts, cleans, and geolocates new WordPress/Gravity Forms sign-ups before committing them to PostgreSQL via `UPSERT` (see the Architecture section above for the full pipeline detail).
...
3. **Spin up the development environment profile:**
...
With your development environment already running (see Quick Start above, or the full walkthrough in [`doc/first_run.md`](https://github.com/maiktreya/tenantsUnion/blob/main/doc/first_run.md)), run the tests and review code coverage matrices with:
...
For comprehensive configuration steps and instructions on running isolated integration passes, review the [Testing documentation](https://github.com/maiktreya/tenantsUnion/blob/main/doc/testing.md).
