...
async def test_successful_login_and_navigation(ui_tester: SeleniumUITester):
    # After the rpc_login refactor (audit item #3a), the login view makes
    # a single POST /rpc/rpc_login call instead of fetching usuarios,
    # usuario_credenciales, usuario_roles and roles separately as web_anon.
    respx.post(f"{REAL_API_URL}/rpc/rpc_login").mock(
    ...
    # Use a dummy password as the login is mocked and does not use a real credential.
    ui_tester.find_and_type("Password", "test-password")
...
async def test_failed_login_shows_error(ui_tester: SeleniumUITester):
    # rpc_login returns an empty array on any failure (unknown user,
    # inactive, no credentials, wrong password). The login view treats
    # every one of these as "wrong username or password" so timing/UI do
    # not leak which condition matched.
    respx.post(f"{REAL_API_URL}/rpc/rpc_login").mock(
