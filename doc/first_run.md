...
- `POSTGRES_USER` / `POSTGRES_DB` / `POSTGRES_DB_SCHEMA`: must match the values expected by `04-populate.sql` and the PostgREST URI in `docker-compose.yaml`.
...
The repo ships with three pre-hashed development users defined in `build/postgreSQL/init-scripts/04-populate.sql`. They are inserted automatically the first time the `db` container initializes an empty data directory — **no manual hash generation is required for local dev**.
...
The bcrypt hashes for these accounts are committed in plain sight inside `04-populate.sql` (`$2b$12$…`). They are **intentionally public** so anyone can boot the dev stack, and they must **never** reach a production deployment. Change them before exposing any instance to the internet.

### Rotating the dev credentials (optional, recommended before any shared demo)

If you want to replace the committed hashes with your own, regenerate them locally and patch the `INSERT INTO usuario_credenciales` block in `build/postgreSQL/init-scripts/04-populate.sql`:
...
Production instances must not rely on `04-populate.sql` at all. Either:

1. Mount a replacement `04-populate.sql` with real admin hashes via a host-side secret, or
...
2. Use the corresponding password committed in `build/postgreSQL/init-scripts/04-populate.sql`. Because the bcrypt hashes are public, ask a teammate or inspect the SQL file for the current dev password.
...
- **Password Hashing:** All credentials are stored as bcrypt hashes via `passlib` (`CryptContext(schemes=["bcrypt"])`). `passlib`'s bcrypt default is **12 rounds** (the `$2b$12$…` prefixes in `04-populate.sql` confirm this). The code does not override the round count.
...
- ✅ Treat the credentials in `04-populate.sql` as throwaway — they are public by design.
